{{-- resources/views/admin/ict-tickets/index.blade.php --}}
{{-- Extend whatever your admin layout is — adjust @extends as needed --}}
@extends('layouts.ict-tickets-admin')

@section('title', 'ICT Tickets')
@section('page-title', 'ICT Ticket Management')
@section('page-desc', 'All submitted ICT Technical Assistance requests')
@section('active-nav', 'ta-tickets')
...
@section('content')
    <div style="padding: 32px 5%;">
        ...
        <div
            ...
            <div>
                <h1
                    ...
                    ICT Ticket Management
                </h1>
                <p style="font-size:13px;color:#6B7A90;">
                    All submitted ICT Technical Assistance requests.
                </p>
            </div>
        ...
        {{-- ── Filters ── --}}
        <form method="GET" action="{{ route('ict.admin.ta-requests.index') }}"
            style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px;">
            <input type="text" name="search" value="{{ request('search') }}"
                placeholder="Search ticket no., name, department…"
            ...
            <table style="width:100%;border-collapse:collapse;">
                <thead>
                    <tr
                        ...
                        @foreach (['Ticket No.', 'Submitted By', 'Department', 'Assistance', 'Date', 'Status', 'Actions'] as $th)
                            <th
                    ...
                    @forelse($tickets as $ticket)
                        <tr style="border-bottom:1px solid rgba(26,74,138,0.07);transition:background .15s;"
                            ...
                            <td style="padding:14px 16px;">
                                <a href="{{ route('ict.admin.ta-requests.show', $ticket) }}"
                            ...
                            <td style="padding:14px 16px;">
                                ...
                                <div style="font-size:11px;color:#6B7A90;">{{ $ticket->position }}</div>
                            </td>
                            ...
                            <td style="padding:14px 16px;">
                                <div style="display:flex;flex-wrap:wrap;gap:4px;">
                                    @foreach ($ticket->assistance_types as $type)
                                        <span
                                            style="font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;background:rgba(37,99,235,0.08);color:#2563eb;border:1px solid rgba(37,99,235,0.14);">
