...
class AttendanceController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            try {
                $parsedDate = Carbon::parse(trim($data['date']))->format('Y-m-d');
            } catch (\Exception $e) {
            ...
            if (! empty($data['gov_email'])) {
                $user = User::where('username', trim($data['gov_email']))->first();
                if ($user) {
                    $employee = Employee::where('user_id', $user->id)->first();
                }
            }
            if (! $employee && ! empty($data['employee_id'])) {
                $user = User::where('user_id', trim($data['employee_id']))->first();
                if ($user) {
                    $employee = Employee::where('user_id', $user->id)->first();
                }
            }
